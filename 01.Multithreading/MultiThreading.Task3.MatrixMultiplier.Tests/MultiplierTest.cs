...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class MultiplierTest
    {
        ...
        public void ParallelEfficiencyTest()
        {
            ...
            // Parallel multuplication more effective started from matrix size equal 55
            //TestEfficiency(10);
            TestEfficiency(55);
        }

        void TestEfficiency(int matrixSize)
        {
            var firstMatrix = new Matrix(matrixSize, matrixSize, true);
            var secondMatrix = new Matrix(matrixSize, matrixSize, true);
            var sequenceProcessResult = GetMatricesMultiplyResult(firstMatrix, secondMatrix, new MatricesMultiplier());
            var parallelProcessResult = GetMatricesMultiplyResult(firstMatrix, secondMatrix, new MatricesMultiplierParallel());

            Assert.IsTrue(parallelProcessResult < sequenceProcessResult);
        }

        long GetMatricesMultiplyResult(IMatrix firstMatrix, IMatrix secondMatrix, IMatricesMultiplier multiplier)
        {
            var stopWatch = new Stopwatch();
            stopWatch.Start();
            var resultMatrix = multiplier.Multiply(firstMatrix, secondMatrix);
            stopWatch.Stop();
